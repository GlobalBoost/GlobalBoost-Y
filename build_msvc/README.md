Building GlobalBoost Core with Visual Studio
========================================

Introduction
---------------------
Visual Studio 2022 is minimum required to build GlobalBoost Core.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build GlobalBoost Core from the command-line, it is sufficient to only install the [Visual Studio Build Tools](https://visualstudio.microsoft.com/downloads/) component.
...
Qt
---------------------
To build GlobalBoost Core with the GUI, a static build of Qt is required.
...
To build GlobalBoost Core without Qt, unload or disable the `globalboost-qt`, `libglobalboost_qt` and `test_globalboost-qt` projects.
...
```cmd
msbuild build_msvc\globalboost.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/globalboost.sln` file in Visual Studio.

Security
---------------------
[Base address randomization](https://learn.microsoft.com/en-us/cpp/build/reference/dynamicbase-use-address-space-layout-randomization) is used to make GlobalBoost Core more secure. When building GlobalBoost using the `build_msvc` process base address randomization can be disabled by editing `common.init.vcproj` to change `RandomizedBaseAddress` from `true` to `false` and then rebuilding the project.

To check if `globalboostd` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/globalboostd.exe
```

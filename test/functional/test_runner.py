...
BASE_SCRIPTS = [
    ...
    'interface_globalboost_cli.py --legacy-wallet',
    'interface_globalboost_cli.py --descriptors',
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if globalboostd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "globalboostd"], stdout=subprocess.DEVNULL).returncode == 0:

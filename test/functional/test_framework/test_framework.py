...
TMPDIR_PREFIX = "globalboost_func_test_"
...
class GlobalBoostTestMetaClass(type):
    """Metaclass for GlobalBoostTestFramework.

    Ensures that any attempt to register a subclass of `GlobalBoostTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'GlobalBoostTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("GlobalBoostTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("GlobalBoostTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class GlobalBoostTestFramework(metaclass=GlobalBoostTestMetaClass):
    ...
    def setup(self):
        ...
        fname_globalboostd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "globalboostd" + config["environment"]["EXEEXT"],
        )
        fname_globalboostcli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "globalboost-cli" + config["environment"]["EXEEXT"],
        )
        fname_globalboostutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "globalboost-util" + config["environment"]["EXEEXT"],
        )
        self.options.globalboostd = os.getenv("GLOBALBOOSTD", default=fname_globalboostd)
        self.options.globalboostcli = os.getenv("GLOBALBOOSTCLI", default=fname_globalboostcli)
        self.options.globalboostutil = os.getenv("GLOBALBOOSTUTIL", default=fname_globalboostutil)
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'globalboostd', self.options.globalboostd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'globalboost-cli', self.options.globalboostcli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                globalboostd=binary[i],
                globalboost_cli=binary_cli[i],
                version=versions[i],
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    globalboostd=self.options.globalboostd,
                    globalboost_cli=self.options.globalboostcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in globalboost.conf
    ...
    def skip_if_no_globalboostd_tracepoints(self):
        """Skip the running test if globalboostd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("globalboostd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_globalboostd_zmq(self):
        """Skip the running test if globalboostd has not been compiled with zmq support."""
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if globalboost-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("globalboost-wallet has not been compiled")

    def skip_if_no_globalboost_util(self):
        """Skip the running test if globalboost-util has not been compiled."""
        if not self.is_globalboost_util_compiled():
            raise SkipTest("globalboost-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if globalboost-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("globalboost-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether globalboost-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether globalboost-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_globalboost_util_compiled(self):
        """Checks whether globalboost-util was compiled."""
        return self.config["components"].getboolean("ENABLE_GLOBALBOOST_UTIL")

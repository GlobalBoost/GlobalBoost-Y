...
AC_DEFUN([GLOBALBOOST_QT_FAIL],[
  if test "$globalboost_qt_want_version" = "auto" && test "$globalboost_qt_force" != "yes"; then
    if test "$globalboost_enable_qt" != "no"; then
...
AC_DEFUN([GLOBALBOOST_QT_CHECK],[
  if test "$globalboost_enable_qt" != "no" && test "$globalboost_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([GLOBALBOOST_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [
     ...
     if test "$globalboost_qt_want_version" = "yes"; then
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); globalboost_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   GLOBALBOOST_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _GLOBALBOOST_QT_FIND_LIBS
...
AC_DEFUN([GLOBALBOOST_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  GLOBALBOOST_QT_CHECK([_GLOBALBOOST_QT_FIND_LIBS])
  ...
  dnl the final binary as well. _GLOBALBOOST_QT_CHECK_STATIC_PLUGIN does
  ...
  if test "$globalboost_cv_static_qt" = "yes"; then
    _GLOBALBOOST_QT_CHECK_STATIC_LIBS
    ...
    if test "$TARGET_OS" != "android"; then
      _GLOBALBOOST_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "$TARGET_OS" = "windows"; then
      ...
      _GLOBALBOOST_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      _GLOBALBOOST_QT_CHECK_STATIC_PLUGIN([QWindowsVistaStylePlugin], [-lqwindowsvistastyle])
      AC_DEFINE([QT_QPA_PLATFORM_WINDOWS], [1], [Define this symbol if the qt platform is windows])
    elif test "$TARGET_OS" = "linux"; then
      _GLOBALBOOST_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _GLOBALBOOST_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _GLOBALBOOST_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  GLOBALBOOST_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  if test $globalboost_enable_qt = "yes"; then
    AC_MSG_RESULT([$globalboost_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$globalboost_enable_qt])
  fi
...
dnl
dnl _GLOBALBOOST_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_GLOBALBOOST_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); GLOBALBOOST_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _GLOBALBOOST_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_GLOBALBOOST_QT_CHECK_STATIC_LIBS], [
...
dnl
dnl _GLOBALBOOST_QT_FIND_LIBS
dnl ---------------------
dnl
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: have_qt_test and have_qt_dbus are set (if applicable) to yes|no.
AC_DEFUN([_GLOBALBOOST_QT_FIND_LIBS],[
  GLOBALBOOST_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_CORE_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_CORE_LIBS $QT_LIBS"],
                      [GLOBALBOOST_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  GLOBALBOOST_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_GUI_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_GUI_LIBS $QT_LIBS"],
                      [GLOBALBOOST_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  GLOBALBOOST_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_WIDGETS_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_WIDGETS_LIBS $QT_LIBS"],
                      [GLOBALBOOST_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  GLOBALBOOST_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_NETWORK_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_NETWORK_LIBS $QT_LIBS"],
                      [GLOBALBOOST_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])

...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing globalboost-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='globalboost-cli', help='globalboost-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='GlobalBoost address to which the faucet should send')
...
parser.add_argument('globalboost_cli_args', nargs='*', help='Arguments to pass on to globalboost-cli (default: -signet)')

args = parser.parse_args()

if args.globalboost_cli_args == []:
    args.globalboost_cli_args = ['-signet']


def globalboost_cli(rpc_command_and_params):
    argv = [args.cmd] + args.globalboost_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = globalboost_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = globalboost_cli(['getnewaddress', 'faucet', 'bech32'])

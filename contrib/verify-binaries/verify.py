...
"""Script for verifying GlobalBoost Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from globalboostcore.org and globalboost.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/globalboost-core/guix.sigs/tree/main/builder-keys
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the GlobalBoost community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"globalboost_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by globalboostcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the globalboost release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes globalboost.org lags behind globalboostcore.org.)')
    )

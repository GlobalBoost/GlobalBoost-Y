# Bootstrappable GlobalBoost Core Builds

This directory contains the files necessary to perform bootstrappable GlobalBoost
Core builds.
...
To build GlobalBoost Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      GlobalBoost Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/globalboost-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/globalboost-core/guix.sigs).

...
""" Example logging GlobalBoost Core mempool events using the mempool:added,
...
def main(globalboostd_path):
    globalboostd_with_usdts = USDT(path=str(globalboostd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    globalboostd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    globalboostd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    globalboostd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    globalboostd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[globalboostd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/globalboostd")
        exit(1)

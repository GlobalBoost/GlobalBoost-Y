...
Tracing (USDT) support for GlobalBoost Core on Linux using. For more information on
USDT support in GlobalBoost Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `globalboostd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `globalboostd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[GlobalBoost Core USDT documentation] on how to list available tracepoints in your
`globalboostd` binary.

[GlobalBoost Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
Output
```
outbound 'ping' msg to peer 11 (outbound-full-relay, [2a02:b10c:f747:1:ef:fake:ipv6:addr]:8226) with 8 bytes
inbound 'pong' msg from peer 11 (outbound-full-relay, [2a02:b10c:f747:1:ef:fake:ipv6:addr]:8226) with 8 bytes
inbound 'inv' msg from peer 16 (outbound-full-relay, XX.XX.XXX.121:8226) with 37 bytes
outbound 'getdata' msg to peer 16 (outbound-full-relay, XX.XX.XXX.121:8226) with 37 bytes
inbound 'tx' msg from peer 16 (outbound-full-relay, XX.XX.XXX.121:8226) with 222 bytes
outbound 'inv' msg to peer 9 (outbound-full-relay, faketorv3addressa2ufa6odvoi3s77j4uegey0xb10csyfyve2t33curbyd.onion:8226) with 37 bytes
outbound 'inv' msg to peer 7 (outbound-full-relay, XX.XX.XXX.242:8226) with 37 bytes
…
```
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/globalboostd
```
...
```
 ...
 PEER  OUTBOUND              INBOUND               TYPE                   ADDR
    0  46          398 byte  61      1407590 byte  block-relay-only       XX.XX.XXX.196:8226
   11  1156     253570 byte  3431    2394924 byte  outbound-full-relay    XXX.X.XX.179:8226
   13  3425    1809620 byte  1236     305458 byte  inbound                XXX.X.X.X:60380
   16  1046     241633 byte  1589    1199220 byte  outbound-full-relay    4faketorv2pbfu7x.onion:8226
   19  577      181679 byte  390      148951 byte  outbound-full-relay    kfake4vctorjv2o2.onion:8226
   20  11         1248 byte  13         1283 byte  block-relay-only       [2600:fake:64d9:b10c:4436:aaaa:fe:bb]:8226
   21  11         1248 byte  13         1299 byte  block-relay-only       XX.XXX.X.155:8226
   22  5           103 byte  1           102 byte  feeler                 XX.XX.XXX.173:8226
   23  11         1248 byte  12         1255 byte  block-relay-only       XX.XXX.XXX.220:8226
   24  3           103 byte  1           102 byte  feeler                 XXX.XXX.XXX.64:8226
…
```
...
```
    ----------------------------------------------------------------------
    |                PEER 16 (4faketorv2pbfu7x.onion:8226)               |
...
GlobalBoost P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/globalboostd
```
...
Some messages might be lost!
 outbound msg 'inv' from peer 4 (outbound-full-relay, XX.XXX.XX.4:8226) with 253 bytes: 0705000000be2245c8f844c9f763748e1a7…
…
Warning: incomplete message (only 32568 out of 53552 bytes)! inbound msg 'tx' from peer 32 (outbound-full-relay, XX.XXX.XXX.43:8226) with 53552 bytes: 020000000001fd3c01939c85ad6756ed9fc…
…
...
In a different terminal, starting GlobalBoost Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/globalboostd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/globalboostd
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/globalboostd
```

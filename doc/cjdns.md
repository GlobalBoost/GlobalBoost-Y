# CJDNS support in GlobalBoost Core

It is possible to run GlobalBoost Core over CJDNS, an encrypted IPv6 network that
...
done outside of GlobalBoost Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the GlobalBoost network and individual nodes.
...
before it will work with your GlobalBoost Core node. This is described in steps
...
## Run GlobalBoost Core with CJDNS

Once you are connected to the CJDNS network, the following GlobalBoost Core
...
When enabled, this option tells GlobalBoost Core that it is running in an
...
IPv6 local network. This helps GlobalBoost Core perform better address management:
...
CJDNS support was added to GlobalBoost Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `globalboost-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/globalboost/globalboost/blob/master/doc/tor.md) for
details.

## CJDNS-related information in GlobalBoost Core

There are several ways to see your CJDNS address in GlobalBoost Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `globalboost-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `globalboost-cli getpeerinfo`).

...
GlobalBoost Core version 0.19.1 is now available from:

  <https://globalboostcore.org/bin/globalboost-core-0.19.1/>
...
From GlobalBoost Core 0.17.0 onwards, macOS versions earlier than 10.10 are no
longer supported, as GlobalBoost Core is now built using Qt 5.9.x which requires
macOS 10.10+. Additionally, GlobalBoost Core does not yet change appearance when
...
- #17488 fix "globalboostd already running" warnings on macOS (fanquake)

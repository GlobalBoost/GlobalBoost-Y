GlobalBoost Core version 0.18.0 is now available from:

  <https://globalboostcore.org/bin/globalboost-core-0.18.0/>
...
`/Applications/GlobalBoost-Qt` (on Mac) or `globalboostd`/`globalboost-qt` (on
Linux).
...
recommended to use GlobalBoost Core on unsupported systems.
...
10.10. Additionally, GlobalBoost Core does not yet change appearance when
...
`contrib/init/globalboostd.service`, it has been changed to use
`/var/lib/globalboostd` as the data directory instead of
`~globalboost/.globalboost`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/globalboostd` will
exist has enough space (check using `df -h /var/lib/globalboostd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/globalboost/globalboost/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:8225:8225` (this is an extra `:8225` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://globalboostcore.org/en/contact/) the GlobalBoost Core
...
The systemd init file (`contrib/init/globalboostd.service`) has been changed
to use `/var/lib/globalboostd` as the data directory instead of
`~globalboost/.globalboost`. This change makes GlobalBoost Core more consistent with
...
`datadir` option in `/etc/globalboost/globalboost.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/globalboost/globalboost.conf`.


Documentation
-------------

- A new short [document](https://github.com/globalboost/globalboost/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/globalboost/globalboost/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/globalboost/globalboost/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/globalboost/globalboost/blob/master/doc/globalboost-conf.md)
  about the `globalboost.conf` file describes how to use it to configure
  GlobalBoost Core.

- A new document introduces GlobalBoost Core's BIP174 [Partially-Signed
  GlobalBoost Transactions
  (PSBT)](https://github.com/globalboost/globalboost/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/globalboost/globalboost/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent GlobalBoost-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  GlobalBoost Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/globalboost/globalboost/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/globalboost/globalboost/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when GlobalBoost Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of GlobalBoost Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/globalboost/globalboost/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/globalboost/globalboost/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart globalboostd with the `-deprecatedrpc=generate`
...
- A new `globalboost-wallet` tool is now distributed alongside GlobalBoost
...
This section describes planned changes to GlobalBoost Core that may affect
other GlobalBoost software and services.

- Since version 0.16.0, GlobalBoost Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with GlobalBoost Core 0.20 (expected about
  a year after 0.18), GlobalBoost Core will default to native segwit
  ...
  to bech32 addresses, and if the GlobalBoost Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in GlobalBoost Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any GlobalBoost Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, GlobalBoost Core
...
- A sub-project of GlobalBoost Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with GlobalBoost Core.  See their [project
  page](https://github.com/globalboost-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to GlobalBoost Core's own implementation, although entropy
  gathered by GlobalBoost Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves GlobalBoost Core a
...
- On macOS, GlobalBoost Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `globalboost_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libglobalboost_server.a` (jnewbery)
...
- #13926 [Tools] globalboost-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like globalboostd (MarcoFalke)
...
- #14521 qt, docs: Fix `globalboost-qt -version` output formatting (hebasto)
...
- #11625 Add GlobalBoostApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libglobalboost_server` (laanwj)
...
- #15154 Configure: globalboost-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/globalboost-wallet (Sjors)
...
- #11526 Visual Studio build configuration for GlobalBoost Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libglobalboost_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in globalboost-util-test.py (isghe)
...
- #12775 Integration of property based testing into GlobalBoost Core (Christewart)
...
- #12246 Bugfix: Only run globalboost-tx tests when globalboost-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_globalboost` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_globalboost` (MarcoFalke)
...
- #14186 globalboost-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_globalboost.conf` message on startup if conf file exists (leishman)
...
- #12255 Update globalboost.service to conform to init.md (dongcarl)
...
- #14497 Add doc/globalboost-conf.md (hebasto)
...
- #15354 Add missing `globalboost-wallet` tool manpages (MarcoFalke)
...
- #15765 correct globalboostconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support globalboost folder on OSX (JimmyMow)

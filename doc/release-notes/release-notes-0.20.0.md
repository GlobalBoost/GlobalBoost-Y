...
GlobalBoost Core version 0.20.0 is now available from:

  <https://globalboostcore.org/bin/globalboost-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from GlobalBoost Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since GlobalBoost Core version 0.18.0.
...
- Testing or debugging of implementations of the GlobalBoost P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of GlobalBoost Core. GlobalBoost Core logs debug messages
...
- OpenSSL is no longer used by GlobalBoost Core.  (#17265)

- BIP70 support has been fully removed from GlobalBoost Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/globalboost/globalboost/issues/16599),
  [PR #16702](https://github.com/globalboost/globalboost/pull/16702), and the
  `globalboostd help` for more information.  This option is experimental and
...
- All custom settings configured when GlobalBoost Core starts are now
...
- The "Start GlobalBoost Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://globalboostcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple GlobalBoost Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed GlobalBoost Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/globalboost-core/HWI) for
  signing.  Future versions of GlobalBoost Core should support a GUI option
...
- GlobalBoost Core's automatically-generated source code documentation is
  now available at https://doxygen.globalboostcore.org.  (#17596)
...
- The `globalboost-cli` utility used with the `-getinfo` parameter now
...
- The `globalboost-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=8226` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `globalbooststrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_globalboost-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBGLOBALBOOST_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_globalboost` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBGLOBALBOOST_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for globalboost-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate globalboost schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest globalboost.conf (sdaftuar)
...
- #18022 Fix appveyor `test_globalboost` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (globalboost-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone globalboost-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about GlobalBoost Core files (hebasto)
...
- #17119 Fix broken globalboost-cli examples (andrewtoth)
...
- #17503 Remove bitness from globalboost-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.globalboostcore.org (laanwj)
...
- #17648 Rename wallet-tool references to globalboost-wallet (hel-o)
...
- #18341 Replace remaining literal BSTY with `CURRENCY_UNIT` (domob1812)
...
- #16115 On globalboostd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from globalboost-cli -getinfo (malevolent)

# PSBT Howto for GlobalBoost Core

Since GlobalBoost Core 0.17, an RPC interface exists for Partially Signed GlobalBoost
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/globalboost/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for GlobalBoost transactions that are not fully signed
...
#### Multisig with multiple GlobalBoost Core instances
...
GlobalBoost Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `globalboost-cli` is used.
...
Later, when *V* BSTY has been received on *Amulti*, and Bob and Carol want to

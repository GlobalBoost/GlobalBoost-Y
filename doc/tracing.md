# User-space, Statically Defined Tracing (USDT) for GlobalBoost Core

GlobalBoost Core includes statically defined tracepoints to allow for more
...
**Note**: GlobalBoost Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to GlobalBoost Core
...
Multiple tools can list the available tracepoints in a `globalboostd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in GlobalBoost Core, use `info probes` in `gdb`:

```
$ gdb ./src/globalboostd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/globalboostd
stap net        outbound_message 0x0000000000107c05 /src/globalboostd
stap validation block_connected  0x00000000002fb10c /src/globalboostd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in GlobalBoost Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/globalboostd | grep NT_STAPSDT -A 4 -B 2

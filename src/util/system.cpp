...
const char * const GLOBALBOOST_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //globalboost-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", GLOBALBOOST_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\GlobalBoost
    // macOS: ~/Library/Application Support/GlobalBoost
    // Unix-like: ~/.globalboost
#ifdef WIN32
    // Windows
...
{
    return GetConfigFile(*this, GetPathArg("-conf", GLOBALBOOST_CONF_FILENAME));
}

...
BOOST_AUTO_TEST_CASE(addrman_simple)
{
    ...
    CService addr1 = ResolveService("250.1.1.1", 8226);
    BOOST_CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 1U);
    auto addr_ret1 = addrman->Select().first;
    BOOST_CHECK_EQUAL(addr_ret1.ToStringAddrPort(), "250.1.1.1:8226");

    // Test: Does IP address deduplication work correctly.
    //  Expected dup IP should not be added.
    CService addr1_dup = ResolveService("250.1.1.1", 8226);
    ...
    CService addr2 = ResolveService("250.1.1.2", 8226);
    ...
    vAddr.push_back(CAddress(ResolveService("250.1.1.3", 8226), NODE_NONE));
    vAddr.push_back(CAddress(ResolveService("250.1.1.4", 8226), NODE_NONE));
...
BOOST_AUTO_TEST_CASE(addrman_ports)
{
    ...
    CService addr1_port = ResolveService("250.1.1.1", 8227);
    BOOST_CHECK(addrman->Add({CAddress(addr1_port, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 2U);
    auto addr_ret2 = addrman->Select().first;
    BOOST_CHECK(addr_ret2.ToStringAddrPort() == "250.1.1.1:8226" || addr_ret2.ToStringAddrPort() == "250.1.1.1:8227");
    ...
    BOOST_CHECK_EQUAL(addr_ret3.ToStringAddrPort(), "250.1.1.1:8226");
}
...
BOOST_AUTO_TEST_CASE(addrman_select)
{
    ...
    CService addr2 = ResolveService("250.3.1.1", 8226);
    CService addr3 = ResolveService("250.3.2.2", 9999);
    CService addr4 = ResolveService("250.3.3.3", 9999);

    BOOST_CHECK(addrman->Add({CAddress(addr2, NODE_NONE)}, ResolveService("250.3.1.1", 8226)));
    BOOST_CHECK(addrman->Add({CAddress(addr3, NODE_NONE)}, ResolveService("250.3.1.1", 8226)));
    BOOST_CHECK(addrman->Add({CAddress(addr4, NODE_NONE)}, ResolveService("250.4.1.1", 8226)));

    // Add three addresses to tried table.
    CService addr5 = ResolveService("250.4.4.4", 8226);
    CService addr6 = ResolveService("250.4.5.5", 7777);
    CService addr7 = ResolveService("250.4.6.6", 8226);

    BOOST_CHECK(addrman->Add({CAddress(addr5, NODE_NONE)}, ResolveService("250.3.1.1", 8226)));
    BOOST_CHECK(addrman->Good(CAddress(addr5, NODE_NONE)));
    BOOST_CHECK(addrman->Add({CAddress(addr6, NODE_NONE)}, ResolveService("250.3.1.1", 8226)));
    BOOST_CHECK(addrman->Good(CAddress(addr6, NODE_NONE)));
    BOOST_CHECK(addrman->Add({CAddress(addr7, NODE_NONE)}, ResolveService("250.1.1.3", 8226)));
...
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CAddress addr{CAddress(ResolveService("253.3.3.3", 8226), NODE_NONE)};
...
BOOST_AUTO_TEST_CASE(addrman_getaddr)
{
    ...
    CAddress addr1 = CAddress(ResolveService("250.250.2.1", 8226), NODE_NONE);
    addr1.nTime = Now<NodeSeconds>(); // Set time so isTerrible = false
    CAddress addr2 = CAddress(ResolveService("250.251.2.2", 9999), NODE_NONE);
    addr2.nTime = Now<NodeSeconds>();
    CAddress addr3 = CAddress(ResolveService("251.252.2.3", 8226), NODE_NONE);
    addr3.nTime = Now<NodeSeconds>();
    CAddress addr4 = CAddress(ResolveService("252.253.3.4", 8226), NODE_NONE);
    addr4.nTime = Now<NodeSeconds>();
    CAddress addr5 = CAddress(ResolveService("252.254.4.5", 8226), NODE_NONE);
...
{
    CAddress addr1 = CAddress(ResolveService("250.1.1.1", 8226), NODE_NONE);
...
{
    CAddress addr1 = CAddress(ResolveService("250.1.2.1", 8226), NODE_NONE);
...
BOOST_AUTO_TEST_CASE(load_addrman)
{
    ...
    BOOST_CHECK(Lookup("250.7.1.1", addr1, 8226, false));
    ...
    // Add three addresses to new table.
    CService source;
    BOOST_CHECK(Lookup("252.5.1.1", source, 8226, false));
...
BOOST_AUTO_TEST_CASE(addrman_update_address)
{
    ...
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CNetAddr source{ResolveIP("252.2.2.2")};
    CAddress addr{CAddress(ResolveService("250.1.1.1", 8226), NODE_NONE)};
    ...
    CAddress addr_diff_port{CAddress(ResolveService("250.1.1.1", 8227), NODE_NONE)};
...
BOOST_AUTO_TEST_CASE(addrman_size)
{
    ...
    const CAddress addr1{ResolveService("250.1.1.1", 8226), NODE_NONE};
    BOOST_CHECK(addrman->Add({addr1}, source));
    BOOST_CHECK(addrman->Good(addr1));
    const CAddress addr2{ResolveService("250.1.1.2", 8226), NODE_NONE};

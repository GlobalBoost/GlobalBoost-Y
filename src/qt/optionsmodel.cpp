...
/** Call node.updateRwSetting() with GlobalBoost 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because GlobalBoost 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/globalboost/globalboost/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because globalboost 22.x will not try to read these.
...
//! Convert enabled/size values to globalboost -prune setting.
...
//! Get pruning enabled value to show in GUI from globalboost -prune setting.
...
//! Get pruning size value to show in GUI from globalboost -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayGlobalBoostUnit")) {
        settings.setValue("DisplayGlobalBoostUnit", QVariant::fromValue(GlobalBoostUnit::BSTY));
    }
    QVariant unit = settings.value("DisplayGlobalBoostUnit");
    if (unit.canConvert<GlobalBoostUnit>()) {
        m_display_globalboost_unit = unit.value<GlobalBoostUnit>();
    } else {
        m_display_globalboost_unit = GlobalBoostUnit::BSTY;
        settings.setValue("DisplayGlobalBoostUnit", QVariant::fromValue(m_display_globalboost_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if globalboost.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_globalboost_unit);
...
{
    if (new_unit.isNull() || new_unit.value<GlobalBoostUnit>() == m_display_globalboost_unit) return;
    m_display_globalboost_unit = new_unit.value<GlobalBoostUnit>();
    QSettings settings;
    settings.setValue("DisplayGlobalBoostUnit", QVariant::fromValue(m_display_globalboost_unit));
    Q_EMIT displayUnitChanged(m_display_globalboost_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    // (https://github.com/globalboost-core/gui/issues/567).

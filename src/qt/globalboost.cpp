...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<GlobalBoostUnit>("GlobalBoostUnit");
#else
    qRegisterMetaType<GlobalBoostUnit>("GlobalBoostUnit");
#endif
}
...
static const char* qt_argv = "globalboost-qt";
...
bool GlobalBoostApplication::createOptionsModel(bool resetSettings)
{
...
{
    window = new GlobalBoostGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &GlobalBoostGUI::quitRequested, this, &GlobalBoostApplication::requestShutdown);
...
void GlobalBoostApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool GlobalBoostApplication::baseInitialize()
{
...
void GlobalBoostApplication::startThread()
{
    ...
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &GlobalBoostApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &GlobalBoostApplication::handleRunawayException);
    connect(this, &GlobalBoostApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &GlobalBoostApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}
...
void GlobalBoostApplication::InitPruneSetting(int64_t prune_MiB)
{
...
void GlobalBoostApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &GlobalBoostGUI::handlePaymentRequest);
            connect(window, &GlobalBoostGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void GlobalBoostApplication::handleNonFatalException(const QString& message)
{
...
bool GlobalBoostApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", GlobalBoostGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6-7. Parse globalboost.conf, determine network, switch to network specific

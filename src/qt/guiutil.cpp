...
bool parseGlobalBoostURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!GlobalBoostUnits::parse(GlobalBoostUnit::BSTY, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatGlobalBoostURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("globalboost:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(GlobalBoostUnits::format(GlobalBoostUnit::BSTY, info.amount, false, GlobalBoostUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("GlobalBoost (%s).lnk", chain));
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("globalboost-%s.desktop", chain));
}

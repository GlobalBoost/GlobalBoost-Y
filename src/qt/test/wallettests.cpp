...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = GlobalBoostUnits::formatWithUnit(unit, expected_balance, false, GlobalBoostUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_globalboost-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_globalboost-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_globalboost-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("globalboost:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_globalboost-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
